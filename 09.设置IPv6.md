...
经过一段时间的测试，将 `Preferred Lifetime` 设置为 `4` 小时，将 `Valid Lifetime` 设置为 `6` 小时较为可靠。  

|参数|值|说明|
|--|--|--|
|Valid Lifetime|`06:00:00`|IPv6 地址最大有效期|
|Preferred Lifetime|`04:00:00`|IPv6 地址建议有效期|
...
|RA Interval|`100-300`|设置 `ND` 通告的时间间隔，时间单位为秒|
|RA Lifetime|`900`|设置 `ND` 通告的生存周期，时间单位为秒|
